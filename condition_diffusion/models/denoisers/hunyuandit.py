...
class AttentionPool(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        pos_embed_len = self.positional_embedding.shape[0]
        
        if expected_len != pos_embed_len:

            if expected_len > pos_embed_len:

                extra_positions = expected_len - pos_embed_len
                last_pos_embed = self.positional_embedding[-1:].repeat(extra_positions, 1)
                pos_embed = torch.cat([self.positional_embedding, last_pos_embed], dim=0)
            else:
                pos_embed = self.positional_embedding[:expected_len]
        else:
        ...
        x, _ = F.multi_head_attention_forward(
            ...
            q_proj_weight=self.q_proj.weight.to(x.dtype),
            k_proj_weight=self.k_proj.weight.to(x.dtype),
            v_proj_weight=self.v_proj.weight.to(x.dtype),
            in_proj_weight=None,
            in_proj_bias=torch.cat([self.q_proj.bias, self.k_proj.bias, self.v_proj.bias]).to(x.dtype),
            bias_k=None,
            bias_v=None,
            add_zero_attn=False,
            dropout_p=0,
            out_proj_weight=self.c_proj.weight.to(x.dtype),
            out_proj_bias=self.c_proj.bias.to(x.dtype),
...
class HunYuanDiTPlain(nn.Module):
    ...
    def forward(self, x, t, contexts, **kwargs):
        ...
        if self.use_attention_pooling:
            ...
            extra_emb = self.extra_embedder(extra_vec)
