...
from dragdec.drag_dataset import DragDataModuleConfig, DragDataModule
...
def build_trainer(cfg, loggers, callbacks):
    """
    Build PyTorch Lightning trainer (following PhysDec pattern)
    """
...
def build_model(cfg):
    """
    Build the drag estimation model with VAE encoder (following PhysDec pattern)
    """
    ...
    model = DragEstimator(
        ...
        embedder=embedder,
        pre_kl=pre_kl,
        cfg=cfg.system.drag_model,
        out_dim=cfg.system.drag_model.out_dim,
        num_latents=cfg.system.drag_model.num_latents,
        width=cfg.system.drag_model.width,
        heads=cfg.system.drag_model.heads,
        init_scale=cfg.system.drag_model.init_scale,
        qkv_bias=cfg.system.drag_model.qkv_bias,
        use_flash=cfg.system.drag_model.use_flash,
        use_checkpoint=cfg.system.drag_model.use_checkpoint,
        vis_mesh=cfg.data.get('vis_mesh', False),
        noisytrain=cfg.system.drag_model.get('noisytrain', False),
        z_scale_factor=cfg.system.drag_model.get('z_scale_factor', 1.0),
        train_eps=cfg.system.drag_model.get('train_eps', 0.0),
        sample_eps=cfg.system.drag_model.get('sample_eps', 0.0),
    )
...
def main(args, extras):
    """
    Main training function for drag coefficient estimation (following PhysDec pattern)
    """
    ...
    datamodule = DragDataModule(cfg.data)
    ...
    if not os.path.exists(os.path.join(cfg.trial_dir, "dragdec")):
        shutil.copytree('./dragdec', os.path.join(cfg.trial_dir, "dragdec"))
        shutil.copy2('./launch_dragdec.py', os.path.join(cfg.trial_dir, "dragdec", "launch_dragdec.py"))
