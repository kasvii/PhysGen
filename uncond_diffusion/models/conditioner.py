...
class NullConditioner(nn.Module):
    """
    A null conditioner that returns empty conditioning for unconditional generation.
    """
    
    def __init__(self):
        super().__init__()
        
    def forward(self, image=None, text=None, mask=None, **kwargs):
        """
        Returns empty conditioning information for unconditional generation.
        
        Args:
            image: Optional image input (ignored)
            text: Optional text input (ignored) 
            mask: Optional mask input (ignored)
            **kwargs: Other optional inputs (ignored)
            
        Returns:
            Empty conditioning dictionary
        """
        return {}
    
    def encode(self, *args, **kwargs):
        """Compatibility method that returns empty conditioning"""
        return {}
        
    def unconditional_embedding(self, batch_size, **kwargs):
        """Returns empty conditioning for unconditional generation"""
        return {}
