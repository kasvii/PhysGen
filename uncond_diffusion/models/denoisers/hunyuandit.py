...
class HunYuanDiTPlain(nn.Module):
    ...
    def from_single_file(
        ...
        config_path,
        device='cuda',
        dtype=torch.float32,  # Changed from float16 to float32 for numerical stability
        ...
        **kwargs,
        ...
        print(f"🔧 HunYuanDiT from_single_file: About to convert to device={device}, dtype={dtype}")
        model.to(device=device, dtype=dtype)
        print(f"🔧 HunYuanDiT from_single_file: Final model dtype = {next(model.parameters()).dtype}")
        return model
    ...
    def forward(self, x, t, **kwargs):
        ...
        device = x.device
        dtype = x.dtype
        cond = torch.zeros(batch_size, self.text_len, self.context_dim, device=device, dtype=dtype)
        ...
        if self.with_decoupled_ca:
            # Create zero additional conditioning for decoupled cross-attention
            zero_additional_cond = torch.zeros(batch_size, self.decoupled_ca_dim, 1024, 
                                               device=device, dtype=dtype)
            additional_cond = self.additional_cond_proj(zero_additional_cond)
