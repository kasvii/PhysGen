        ...
        if ckpt_path is not None:
            ...
            for k, v in state_dict.items():
                if k.startswith('model.'):
                    ...
                    final_state_dict[k.replace('model.', '')] = v.float()
                else:
                    # Convert weights to float32 for numerical stability
                    final_state_dict[k] = v.float()
            missing, unexpected = self.load_state_dict(final_state_dict, strict=False)
            print('unexpected keys:', unexpected)
            print('missing keys:', missing)
            print('✅ Converted all denoiser weights to float32 for stability')
        else:
            # No checkpoint, but ensure model is initialized in float32
            print('🔧 No checkpoint loaded, initializing model in float32')
        
        # Ensure all model parameters are float32 for numerical stability
        self.float()
        
        # Double-check and force convert any remaining non-float32 parameters
        for name, param in self.named_parameters():
            if param.dtype != torch.float32:
                print(f"⚠️  Converting {name} from {param.dtype} to float32")
                param.data = param.data.float()
        
        print(f"🔧 Hunyuan3DDiT model dtype: {next(self.parameters()).dtype}")

    def forward(self, x, t, contexts=None, **kwargs) -> Tensor:
