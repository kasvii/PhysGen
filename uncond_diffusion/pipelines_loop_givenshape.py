...
class Hunyuan3DDiTPipeline:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.drag_estimator is not None:
            print("DRAG DECODER is usable!.")
        else:
            print("DRAG DECODER is NOT usable!.")
        if self.phys_decoder is not None:
            print("PHYS DECODER is usable!.")
        else:
            print("PHYS DECODER is NOT usable!.")
        self.kwargs = kwargs
        ...
        # Ensure conditioner is in eval mode and doesn't require gradients (following flow_matching_sit.py pattern)
    ...
    def components(self):
        """
        Returns a dictionary of all components of the pipeline.
        """
        return {
            "vae": self.vae,
            "model": self.model,
            ...
            "drag_estimator": self.drag_estimator,
            "phys_decoder": self.phys_decoder,
        }
    ...
    def prepare_image(self, image, mask=None) -> dict:
        ...
        # Handle the case where image_processor returns tensors directly
        if len(outputs) > 0 and isinstance(outputs[0], torch.Tensor):
            return {'image': torch.stack(outputs)}
        ...
        for key, value in cond_input.items():
            cond_input[key] = torch.stack(value) if isinstance(value[0], torch.Tensor) else value
    ...
    def _query_vae_pipeline(self, queries, decoded_latents):
        ...
        # This is a placeholder that should be adapted based on your VAE structure
        if hasattr(self, 'vae') and hasattr(self.vae, 'surface_extractor'):
            try:
                # This would need to be adapted based on the specific VAE implementation
...
class Hunyuan3DDiTFlowMatchingGuidanceLoopPipeline(Hunyuan3DDiTPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Handle condition_image parameter (for consistency with flow_matching_sit.py)
        if condition_image is not None:
            image = condition_image
        
        # Handle arbitrary condition keys from kwargs (e.g., custom cond_stage_key)
        condition_data = None
        ...
        if image is None and mask is None:
            ...
            if self.conditioner is None:
                print("Warning: conditioner is None, treating as unconditional generation")
                cond = None
            else:
        ...
        # Use initial_latents if provided, otherwise prepare random latents
        ...
        if uid is not None:
            # Clean uid for use in filename (remove special characters)
        ...
        with open(sampling_log_path, 'w') as f:
            ...
            f.write(f"Initial drag: {init_drag}\n")
        ...
        for opt_step in tqdm(range(num_optimization_loops), desc="Alternating Optimization"):
            ...
            # Perform constraint optimization for aerodynamic properties
            if self.phys_decoder is not None and opt_step < num_optimization_loops - 1:
                ...
                step = 'ori'
                # Create filename based on optimization_log base name with step number
                ...
                mesh_filename = f"{optimization_log_basename}_{timestamp}_ori.obj"
            ...
            with synchronize_timer('Diffusion Sampling'):
                ...
                for i, t in enumerate(tqdm(timesteps, desc="Diffusion Sampling", disable=True)):
                    ...
                    noise_pred = self.model(latent_model_input, timestep, guidance=guidance)
        ...
        if self.drag_estimator is not None:
            try:
                with torch.inference_mode(False), torch.cuda.amp.autocast(enabled=False):
                    # Create dummy timestep for drag estimation (use 0.0 as default for final evaluation)
            ...
            print("### DRAG ESTIMATOR NOT AVAILABLE - CANNOT COMPUTE FINAL DRAG COEFFICIENT ###")
            with open(sampling_log_path, 'a') as f:
                f.write(f"\n=== Final Drag Coefficient ===\n")
    ...
    def _calculate_face_areas(self, vertices, faces):
        ...
        # Calculate cross product and get magnitude for area
    ...
    def _find_faces_by_direction(self, face_normals, target_direction, angle_threshold_deg=10.0):
        """Find faces that are approximately aligned with target direction"""
        angle_threshold_rad = torch.deg2rad(torch.tensor(angle_threshold_deg, device=face_normals.device))
        cos_threshold = torch.cos(angle_threshold_rad)
        
        # Calculate dot product between face normals and target direction
        dot_products = torch.matmul(face_normals, target_direction)
        
        # Find faces where dot product is greater than cos_threshold
        aligned_faces = dot_products >= cos_threshold
    ...
    def _calculate_face_pressures(self, vertices, faces, pressure_pred):
        ...
        """
        # Prepare pressure tensor and ensure gradient-friendly ops
        ...
        # Use float32 during gather to avoid half-precision backward issues on some GPUs
        ...
        N, C = pressure.shape
        # Build gather indices for each vertex of the face
    ...
    def _optimize_aerodynamic_constraints(self, latents, vertices, faces, pressure_pred, 
        ...
        """
        Optimize latents to satisfy aerodynamic constraints using two possible methods:
        
        Method 1 (Preferred): Direct drag coefficient optimization
        - Uses drag_estimator to predict Cd directly from latents
        - Minimizes: Cd = D / (0.5 * ρ * V² * A)
        - More accurate as it accounts for all drag components and geometry changes
        
        Method 2 (Fallback): Pressure force integration
        - Computes drag force from pressure distribution: F = ∫ P * (n · d) * dA  
        - Minimizes: |F_drag| (absolute drag force)
        - May not account for friction drag and reference area changes
        ...
        # Calculate face normals and areas (static for the mesh)
        ...
        # Create optimizable latents (use fp32 for stable grads)
        ...
        # Ensure physics decoder is in training mode for gradient computation
        if self.phys_decoder is not None:
            self.phys_decoder.train()
            # Enable gradients for physics decoder parameters
            for param in self.phys_decoder.parameters():
                param.requires_grad_(False)
        
        # Define target directions for force integration
        ...
        # Temporarily disable gradient checkpointing inside phys_decoder for stable autograd
        ckpt_originals = {}
        if self.phys_decoder is not None:
            try:
        ...
        with torch.enable_grad():
            for step in range(num_optimization_steps):
                ...
                # Forward pass through physics decoder with explicit gradient handling
                try:
                    with torch.enable_grad():
                ...
                if self.drag_estimator is not None:
                    try:
                        # Predict drag coefficient directly from current latents
                ...
                # Lateral force: force integral in y direction (minimize absolute value for symmetry)
                ...
                # Lift: force integral in z direction (penalize positive lift using ReLU)
                ...
                if self.drag_estimator is not None and 'predicted_cd' in locals():
                    log_str = f"Method: Direct Cd optimization | Drag Loss (Cd): {drag_loss.item():.6f}"
                    log_str += f", Front/Back: {frontback_force.item():.4f}"
                    log_str += f", Lateral: {lateral_force.item():.4f}, Lift: {lift_force.item():.4f}"
                else:
                    log_str = f"Method: Pressure integration | Forces - Drag: {drag_force.item():.4f}"
                    log_str += f", Front/Back: {frontback_force.item():.4f}"
                    log_str += f", Lateral: {lateral_force.item():.4f}, Lift: {lift_force.item():.4f}"
                ...
                # Compute grad wrt optimized_latents exactly once to avoid checkpoint ctx reuse
                grad = None
                try:
                ...
                if grad is None:
                    try:
                        ...
                        # Recompute losses using the same method as main computation
                        if self.drag_estimator is not None:
                            try:
                                dummy_timestep = torch.tensor([1.0], device=optimized_latents.device, dtype=torch.float32)
                                dummy_timestep = dummy_timestep.expand(optimized_latents.shape[0])
                ...
                # Update mesh geometry after optimization step for next iteration
                # This ensures the next forward pass uses the updated mesh
                with torch.no_grad():  # Ensure mesh extraction doesn't interfere with gradients
                    try:
                        meshes = self.extract_geometry_by_diffdmc(
                        ...
                        if meshes[0] is not None:
                            ...
                            # Recalculate face normals and areas for next iteration
                            face_normals = self._calculate_face_normals(vertices_tensor, faces_tensor)
                            face_areas = self._calculate_face_areas(vertices_tensor, faces_tensor)
                            
                            # Save mesh every 2 steps with batch and step info in filename
                            if step % 2 == 0 and optimization_log_path is not None and False:
                                try:
                                    ...
                                    # Add physics prediction if physics decoder is available
                                    #         # Save mesh with physics prediction as VTP file
                                    #             mesh_obj, optimized_latents, physics_filepath, step
                                    #         print(f"  Step {step}: Warning - Failed to add physics prediction: {physics_error}")
                        ...
                        print("  Keeping previous mesh geometry for next iteration")
        ...
        # Compute final drag coefficient using drag estimator if available
    ...
    def _save_mesh_with_physics(self, mesh_obj, optimized_latents, save_filepath, step):
        """
        Save mesh with physics pressure prediction as VTP file
        
        Args:
            mesh_obj: trimesh.Trimesh object
            optimized_latents: Current optimized latents from aerodynamic optimization
            save_filepath: Path to save the VTP file
            step: Current optimization step
        """
        if not PYVISTA_AVAILABLE:
            print(f"  Step {step}: Warning - pyvista not available, cannot save physics mesh")
            return False
            
        if self.phys_decoder is None:
            print(f"  Step {step}: Warning - physics decoder not available")
            return False
        ...
        try:
            ...
            phys_points = torch.from_numpy(vertices).unsqueeze(0).to(
                optimized_latents.device, dtype=torch.float32
            )
            
            with torch.no_grad():
                # First decode the optimized latents using physics decoder  
                physics_latents = self.phys_decoder.decode(optimized_latents)
                
                pressure_pred = self.phys_decoder.query(phys_points, physics_latents)
                
                # Denormalize pressure (following pressure_net.py configuration)
                pressure_std = torch.tensor(self.phys_decoder.cfg.get('PRESSURE_STD', 117.25), 
                                          device=pressure_pred.device, dtype=torch.float32)
                pressure_mean = torch.tensor(self.phys_decoder.cfg.get('PRESSURE_MEAN', -94.5), 
                                           device=pressure_pred.device, dtype=torch.float32)
                pressure_pred = (pressure_std * pressure_pred + pressure_mean)
                
                pressures = pressure_pred.squeeze().cpu().numpy().astype(np.float32)
            
            # Build PyVista faces format (faces need to be prefixed with number of vertices per face)
            ...
            pv_mesh.point_data["pressure"] = pressures
            
            os.makedirs(os.path.dirname(save_filepath), exist_ok=True)
            
            # Save mesh with physics data (.vtp format for PyVista)
            pv_mesh.save(save_filepath)
            
            return True
            
        except Exception as e:
            print(f"  Step {step}: Error saving mesh with physics: {e}")
            return False


    def extract_geometry_by_diffdmc(
        self, 
        latents: torch.Tensor, 
        resolution: int = 256,
        bounds: Union[float, List[float]] = 1.05,
        batch_size: int = 20000,
        verbose: bool = False,
        save_slice_dir: str = None
    ) -> List:
        """
        Extract mesh geometry from latents using DiffDMC (Differentiable Dual Marching Cubes)
        
        Args:
            latents: [B, ...] latent representations
            resolution: grid resolution for mesh extraction
            bounds: bounding box for mesh extraction
            batch_size: batch size for occupancy queries
            verbose: whether to print verbose output
            save_slice_dir: directory to save slice visualizations for debugging
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    def trange(*args, **kwargs):
                        return range(args[1] if len(args) > 1 else args[0])
    ...
    def _save_slice_visualization(self, grid_logits, grid_size, save_slice_dir, verbose=True):
        ...
        try:
            ...
            if verbose:
                print(f"Failed to save slice visualization: {e}")
